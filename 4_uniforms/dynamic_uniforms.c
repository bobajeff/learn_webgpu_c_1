...
int main(int argc, char *argv[]) {
	...
	// Subtlety
	// uint32_t uniformStride = fmax((double)sizeof(MyUniforms), (double)deviceLimits.minStorageBufferOffsetAlignment);
	printf("sizeof(MyUniforms):%lu\n", sizeof(MyUniforms));
	printf("deviceLimits.minUniformBufferOffsetAlignment:%u\n", deviceLimits.minUniformBufferOffsetAlignment);
	uint32_t uniformStride = (uint32_t)sizeof(MyUniforms) > (uint32_t)deviceLimits.minUniformBufferOffsetAlignment ? sizeof(MyUniforms) : deviceLimits.minUniformBufferOffsetAlignment;
	printf("uniformStride: %u\n", uniformStride);
